...
# SVM (usando validação cruzada)
best_svm_rbf = SVC(C=50, kernel='rbf', probability=True)
svm_scores = cross_val_score(best_svm_rbf, X_train, y_train, cv=n_folds)
print(f"SVM Accuracy (CV): {svm_scores.mean()} ± {svm_scores.std()}")

# XGBoost (booster='gblinear') com validação cruzada
...
xgb_scores = cross_val_score(best_xgb_model, X_train, y_train, cv=n_folds)
print(f"XGBoost (gblinear) Accuracy (CV): {xgb_scores.mean()} ± {xgb_scores.std()}")

# Treinar os modelos (SVM, XGBoost, RandomForest)
best_svm_rbf.fit(X_train_split, y_train_split)
...
xgb_model = XGBClassifier(booster='dart', colsample_bytree=0.6, gamma=0.1, learning_rate=0.2,
                          ...
                          reg_alpha=0.01, reg_lambda=10, subsample=0.6, eval_metric='mlogloss')
xgb_model.fit(X_train_split, y_train_split)

# RandomForest com validação cruzada
rf_model = RandomForestClassifier(n_estimators=75, max_depth=None, min_samples_split=10,
                                   min_samples_leaf=4, random_state=2022)
rf_scores = cross_val_score(rf_model, X_train, y_train, cv=n_folds)
print(f"RandomForest Accuracy (CV): {rf_scores.mean()} ± {rf_scores.std()}")
...
print("\nSubmissão salva como Smart-Ensemble-Sequential.csv")
