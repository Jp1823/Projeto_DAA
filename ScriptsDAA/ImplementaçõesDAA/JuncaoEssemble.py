...
# Prepare dictionaries for models and weights
models = {
...
# Define class-to-model mapping
class_model_mapping = {
    0: 'xgb_R',  # Best model for class 0
    1: 'xgb_R',  # Best model for class 1
    2: 'xgb_R',  # Best model for class 2
    3: 'svm_rbf',  # Best model for class 3
    4: 'svm_rbf'   # Best model for class 4
}

# Predict and aggregate probabilities with weights and class mapping
probabilities = {}
for name, model in models.items():
    try:
        probabilities[name] = model.predict_proba(X_test_split)
    except Exception as e:
        print(f"Erro ao gerar probabilidades para o modelo {name}: {e}")

# Final predictions
final_predictions = []

for i in range(len(X_test_split)):
    class_probs = {class_label: 0 for class_label in class_model_mapping.keys()}  # Initialize class probabilities
    
    for class_label, model_name in class_model_mapping.items():
        # Retrieve probabilities for the specific class from the corresponding model
        prob = probabilities.get(model_name, [[0] * len(class_model_mapping)])[i][class_label]
        
        # Weight the probability using the global model weight
        weight = model_weights[model_name]
        class_probs[class_label] += prob * weight
...
# Evaluate the unified ensemble
from sklearn.metrics import classification_report, accuracy_score

print("Relatório de Classificação do Ensemble Unificado (Ajustado):")
print(classification_report(y_test_split, final_predictions, target_names=list(mapping.keys()), zero_division=1))
print("Accuracy do Ensemble Unificado:", accuracy_score(y_test_split, final_predictions))

# --- Final Test Predictions ---

# Apply the same logic to the final test set
...
for i in range(len(X_test_final)):
    class_probs = {class_label: 0 for class_label in class_model_mapping.keys()}
...
submission_df.to_csv('Smart-Ensemble-Weighted-Class.csv', index=False)
